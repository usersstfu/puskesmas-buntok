...
<body>
    ...
    <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
        ...
        <div class="body-wrapper">
            ...
            <br>
            <br>
            <div class="room-container" id="poli_umum">
                @if (session('error'))
                            ...
                            <div class="card-body text-center">
                                <div class="bg-primary rounded-circle d-inline-block p-3">
                                    <h2 class="text-white m-0" id="nomor_antrian">
                                        @if ($currentQueue && $currentQueue->ruangan === 'poli_umum')
                                            {{ $currentQueue->nomor }}
                                        @else
                                            0
                                        @endif
                                ...
                                <h3 class="card-title mt-3">Poli Umum</h3>
                                ...
                                <button class="btn btn-info mb-2" onclick="toggleBerkas()">Lihat Status Berkas</button>
                                ...
                                @if (!$currentQueueNumber)
                                    <form method="POST"
                                        action="{{ route('admin.startQueuePoliUmum', ['ruangan' => 'poli_umum']) }}">
                                        @csrf
                                ...
                                @else
                                    ...
                                    <form method="POST"
                                        action="{{ route('admin.selesaikanAntrianPoliUmum', ['ruangan' => 'poli_umum']) }}"
                                        class="d-flex flex-column mt-3">
                                        @csrf
                                        <input type="hidden" name="nomor_antrian" value="{{ $currentQueue->id }}">
                        ...
                        <div class="card mb-4">
                            <div class="card-body text-center">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody id="poli_umum_queue_body">
                                        @if (isset($allQueues['poli_umum']))
                                            @foreach ($allQueues['poli_umum'] as $queue)
                                                <tr>
                                                    <td>{{ $queue->nomor }}</td>
                                                    <td>{{ $queue->nama }}</td>
                                                    <td>
