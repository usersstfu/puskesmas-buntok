...
<body>
    ...
    <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
        ...
        <div class="body-wrapper">
            ...
            <br>
            <br>
            <div class="room-container" id="poli_gigi">
                @if (session('error'))
                            ...
                            <div class="card-body text-center">
                                <div class="bg-primary rounded-circle d-inline-block p-3">
                                    <h2 class="text-white m-0" id="nomor_antrian">
                                        @if ($currentQueue && $currentQueue->ruangan === 'poli_gigi')
                                            {{ $currentQueue->nomor }}
                                        @else
                                            0
                                        @endif
                                ...
                                <h3 class="card-title mt-3">Poli Gigi</h3>
                                ...
                                @if (!$currentQueueNumber)
                                    <form method="POST"
                                        action="{{ route('admin.startQueuePoliGigi', ['ruangan' => 'poli_gigi']) }}">
                                        @csrf
                                ...
                                @else
                                    ...
                                    <form method="POST"
                                        action="{{ route('admin.selesaikanAntrianPoliGigi', ['ruangan' => 'poli_gigi']) }}"
                                        class="d-flex flex-column mt-3">
                                        @csrf
                                        <input type="hidden" name="nomor_antrian" value="{{ $currentQueue->id }}">
                        ...
                        <div class="card mb-4">
                            <div class="card-body text-center">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody id="poli_gigi_queue_body">
                                        @if (isset($allQueues['poli_gigi']))
                                            @foreach ($allQueues['poli_gigi'] as $queue)
                                                <tr>
                                                    <td>{{ $queue->nomor }}</td>
                                                    <td>{{ $queue->nama }}</td>
                                                    <td>
